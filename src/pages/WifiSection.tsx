...
export default function WifiSection({ provisioning = false }: WifiSectionProps) {
  ...
  const submitApCredentials = useCallback(
    ...
    [apIp, apPassword, apSsid, connected, send],
  );
  ...
  useEffect(() => {
    ...
    void request<Record<string, unknown>>(
      ...
      .then((data) => {
        ...
        const savedStationSsid =
          readAnyString(data.staSsid) ?? readAnyString(data.ssid) ?? "";
        const savedApSsid = readAnyString(data.apSsid) ?? "AutoMicro";
        const savedStationIp =
          readAnyString(data.staConfiguredIp) ?? "0.0.0.0";
        const savedStationGateway =
          readAnyString(data.staGateway) ?? "0.0.0.0";
        const savedStationSubnet =
          readAnyString(data.staSubnet) ?? "255.255.255.0";
        const savedApIp = readAnyString(data.apConfiguredIp) ?? "0.0.0.0";
  ...
  useEffect(() => {
    const offDeviceEvent = subscribe("device.event", (message: unknown) => {
      ...
      if (eventName === "wifi.ap.credentials.result") {
        const success = data.success === true || data.status === "success";
        const reason = readString(data.reason);
        setApSaving(false);
        if (success) {
          const persistedApSsid = readString(data.ssid) ?? apSsid;
          const persistedApIp = readString(data.configuredIp) ?? apIp;
          setApPassword("");
          setPersistedWifiConfig((current) => ({
            ...current,
            loaded: true,
            apSsid: persistedApSsid,
            apIp: persistedApIp,
          }));
        }
        setApFeedback({
          ...
          text: success
            ? `Configuración AP guardada para ${readString(data.ssid) ?? apSsid}. Usa el botón de cambio de modo para reiniciar la ESP como AP.`
            : `No se pudo aplicar la configuración AP${reason ? `: ${reason}` : "."}`,
        });
      }
    });
                  ...
                  <ul className="wifi-network-list wifi-ap-client-list">
                    {apClients.map((client) => {
                      ...
                      return (
                        <li
                          key={client.mac}
                          className={`flex flex-col gap-3 rounded-lg border bg-slate-950/45 px-4 py-3 sm:flex-row sm:items-center sm:justify-between ${isCurrentDevice ? "border-cyan-300/45" : "border-white/10"}`}
                        >
...
function WifiModeSwitchCard({
  ...
  return (
    <section className="mb-5 mt-4 rounded-xl border border-cyan-300/20 bg-slate-950/45 p-4 shadow-[0_18px_50px_rgba(0,0,0,0.22)]">
