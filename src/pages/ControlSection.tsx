...
export default function ControlSection() {
  ...
  return (
    <section
      className="control-dashboard-shell min-h-screen w-full overflow-x-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-6 text-slate-100 selection:bg-cyan-500/30"
    >
      <div className="control-dashboard-frame mx-auto flex w-full max-w-7xl flex-col gap-6">
        <PageHeader
        ...
        <div className={`app-panel-strong relative overflow-hidden px-5 pb-5 pt-14 md:px-7 ${sectionToneClass("cyan")}`}>
          ...
          <div className="pointer-events-none absolute left-1/2 top-3 z-10 -translate-x-1/2">
            <div className="pointer-events-auto flex items-center gap-1 rounded-full border border-cyan-300/20 bg-slate-950/90 p-1 shadow-[0_24px_60px_rgba(0,0,0,0.42)] backdrop-blur-xl">
              <button
                type="button"
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            <ul className="space-y-2 text-sm text-slate-300">
              ...
              <li>
                <span className="font-semibold text-white">Ejecución física:</span>{" "}
...
function BlockPalettePanel({
  ...
  return (
    <section className="mt-5 rounded-lg border border-cyan-300/18 bg-slate-950/45 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
...
function PaletteToolButton({
  ...
  return (
    ...
    >
      <span className="flex size-9 shrink-0 items-center justify-center rounded-md border border-current/30 bg-black/20">
...
function PropertyReadOnly({
  ...
  return (
    <div className="grid grid-cols-[24px_1fr_1.25fr] items-center gap-3 text-sm">
      ...
      <span className="text-slate-400">{label}</span>
      <span className="rounded-md border border-white/10 bg-white/[0.04] px-3 py-2 text-slate-100">
        {value}
      </span>
    </div>
  );
...
function MotionSimulationPanel({
  ...
  return (
    <section className={`${isLarge ? "" : "mb-4"} overflow-hidden rounded-lg border border-cyan-300/20 bg-slate-950/55 ${isLarge ? "p-4" : "p-3"} shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]`}>
      <div className={`grid gap-3 ${isLarge ? "lg:grid-cols-[minmax(0,1fr)_250px]" : "lg:grid-cols-[minmax(0,1fr)_190px]"}`}>
        <div className={`relative overflow-hidden rounded-lg border border-cyan-300/20 bg-slate-950/70 ${isLarge ? "min-h-[340px] md:min-h-[430px]" : "min-h-[150px]"}`}>
          <MotorTopView3D
            modelUrl={modelUrl}
            pose={pose}
            isSimulating={isSimulating}
          ...
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute left-3 top-3 rounded-md border border-amber-300/40 bg-slate-950/85 px-2.5 py-1.5 text-[10px] font-black uppercase tracking-wide text-amber-200 shadow-[0_0_18px_rgba(245,158,11,0.16)] backdrop-blur-sm">
        ...
        <div className="grid content-start gap-2">
          <div>
            ...
            <h3 className={isLarge ? "text-xl font-black text-white" : "text-base font-black text-white"}>
              Vista superior 3D
            </h3>
...
function MetricCard({
  ...
  return (
    <div className="rounded-md border border-white/10 bg-white/[0.04] p-2">
      ...
      <div className={tone === "cyan" ? "mt-0.5 text-sm font-black text-cyan-200" : "mt-0.5 text-sm font-black text-slate-200"}>
        {value}
      </div>
    </div>
  );
    ...
    <aside className={`${sectionCardClass("indigo")} px-4 py-4 md:px-5`}>
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="text-xl font-black text-white">Estado de ejecución</h3>
        </div>
        <StatusPill
          label="Física"
...
function DiagnosticRow({
  ...
  return (
    <div className="flex min-h-[48px] items-center justify-between gap-3 rounded-md border border-white/10 bg-white/[0.025] px-3 py-2">
      <span className="text-slate-400">{label}</span>
      <span className="flex items-center gap-2 text-right font-semibold text-slate-100">
        {value}
