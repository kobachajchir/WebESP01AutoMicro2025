              ...
              <>
                ...
                <div className="grid gap-4 xl:grid-cols-2">
                  {scanResult.validFrames.map((item, index) => {
                    ...
                    const warningsExpanded = expandedWarnings[cardKey] ?? false;

                    return (
                      <article key={cardKey} className="rounded-2xl border border-white/10 bg-slate-950/50 p-4">
                        <div className="flex flex-wrap items-center justify-between gap-3">
                          <h3 className="font-semibold text-white">#{index + 1} @ offset {item.offset}</h3>
                          <div className="flex flex-wrap items-center gap-3">
                            ...
                            {hasWarnings ? (
                              ...
                              >
                                {warningsExpanded ? "Ocultar advertencias" : "Ver advertencias"}
                              </button>
                            ) : null}
                          </div>
                        </div>
                        ...
                        {hasWarnings && warningsExpanded ? (
                          <div className="mt-4 space-y-3">
                            <div className={SMALL_LABEL_CLASS}>Advertencias</div>
                            {warningItems.map((warning, warningIndex) => (
                              <ValidationRow key={`${cardKey}-warning-${warningIndex}`} item={warning} />
                            ))}
                          </div>
                        ) : null}
                        <div className="mt-4">
